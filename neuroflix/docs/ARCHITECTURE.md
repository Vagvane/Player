...
- Proxies all HLS file requests from R2 — the browser never talks to R2 directly
...
Backend
  ...
  └── Returns HLS master playlist URL: /api/v1/videos/{id}/hls/master.m3u8
...
R2 is never exposed directly to the browser. The backend acts as a proxy.
...
The master playlist is generated by the backend and not stored in R2.
...
| R2 exposure | Backend-proxied HLS — browser has no R2 credentials |
